...
char* indexes(const char* path)
{
    ...
    int isThereIndex = 0; //0 indicates no file, 1 indicates .html, 2 indicates .php
    //check if file exists
    FILE *exists = fopen("index.html", "r");
    if (exists)
    {
        isThereIndex = 1;
        fclose(exists);
    ...
    if (isThereIndex == 1) 
    {
        returnValue = malloc((pathLength + 10) *sizeof(char));
        returnValue = strcpy(returnValue, path);
        returnValue = strcat(returnValue, "index.html");
        return returnValue;
    }
    
    else if (isThereIndex == 2)
    {
        returnValue = malloc((pathLength + 9) *sizeof(char));
        returnValue = strcpy(returnValue, path);
        returnValue = strcat(returnValue, "index.php");
        return returnValue;
    }
 ...
 *length = 0;
 ...
 content = malloc(BYTES * sizeof(BYTE));
    while ( c != EOF )
    {
        if ( byteCounter == 512 )
        {
         *length = *length + byteCounter;
         byteCounter = 0;
         content = realloc(content, *length + BYTES );
        }
        
        c = fgetc(file);
        memcpy (content + (*length + byteCounter), &c, 1);
        
    }

 if ( byteCounter < 512 )
 {
     *length = *length - (512 - byteCounter);
     content = realloc(content, *length - (512 - byteCounter));
 }
...
{
    // TODO
    int pathSizeCheck = strlen(path);
    char c = '\0';
    char exten[4] = {0,0,0,0};
    int iter = 0;
    
    while ( c != '.' && iter <= pathSizeCheck )
    {
     c = path[iter];
        if ( c == '.' )
        {
            for (int i = iter, j = 0; i < iter+4; i++, j++)
            {
             exten[j] = path[i + 1];
            }
            
            if ( exten[0] == 'c' && exten[1] == 's' && exten[2] == 's' )
            {
             return "text/css";
            }
            
            if ( exten[0] == 'h' && exten[1] == 't' && exten[2] == 'm' && exten[3] == 'l')
            {
             return "text/html";
            }
            
            if ( exten[0] == 'g' && exten[1] == 'i' && exten[2] == 'f' )
            {
             return "image/gif";
            }
            
            if ( exten[0] == 'i' && exten[1] == 'c' && exten[2] == 'o' )
            {
             return "image/x-icon";
            }
            
            if ( exten[0] == 'j' && exten[1] == 'p' && exten[2] == 'g' )
            {
             return "image/jpeg";
            }
            
            if ( exten[0] == 'j' && exten[1] == 's' )
            {
             return "text/javascript";
            }
            
            if ( exten[0] == 'p' && exten[1] == 'h' && exten[2] == 'p' )
            {
             return "text/x-php";
            }
            
            if ( exten[0] == 'p' && exten[1] == 'n' && exten[2] == 'g' )
            {
             return "image/png";
            }
        }
...
bool parse(const char* line, char* abs_path, char* query)
{
 ...
 //variables
 int spaceChecker = 0;
 int i1stSP = 0; // var for index of first SP
 int i2ndSP = 0; // var for index of second SP
 bool hasQuery = false;
 int RNIndex = strlen(line) - 2;
 
    //block to ensure line contains 2 spaces exactly
    if (spaceChecker == 0)
    {
    ...
    //error call for other than 2 spaces exactly
    if (spaceChecker != 2)
    {
        error(400);
        return false;
    ...
    //block to ensure that method is GET + error calls if not
    for (int methodCheckIter = 0; methodCheckIter < 4; methodCheckIter++)
    {
        if ( methodCheckIter == 0 && line[methodCheckIter] != 'G' )
        {
            error(405);
            return false;
        }
        
        if ( methodCheckIter == 1 && line[methodCheckIter] != 'E' )
        {
            error(405);
            return false;
        }
        
        if ( methodCheckIter == 2 && line[methodCheckIter] != 'T' )
        {
            error(405);
            return false;
        }
        
        if ( methodCheckIter == 3 && line[methodCheckIter] != (char) 32 )
        {
            error(405);
            return false;
        }
    ...
    //Block to look for SP's and assign their indexes to appropriate var
    if (i1stSP == 0) 
    {
        for (int i = 0, j = strlen(line); i < j; i++)
        {
            if ( line[i] == (char) 32 && i1stSP == 0 )
            {
             i1stSP = i;
            }
            
            else if ( line[i] == (char) 32 && i1stSP != 0 )
            {
             i2ndSP = i;
            }
        }
    ...
    //Block to check for \r\n
    {
        for (int i = 1; i < 2; i++)
        {
            if (i == 0 && line[ RNIndex + i] != '\r')
            {
                error(400);
                return false;
            }
            else if (i == 1 && line[RNIndex + i] != '\n')
            {
                error(400);
                return false;
            }
    ...
    //Block to check for request-target
    if (line[i1stSP + 1] != '/')
    {
        error(501);
        return false;
    ...
    //Block to iterate over request-target looking for query and assign query
    {
        for ( int i = i1stSP; i < i2ndSP; i++ )
        {
            if ( line[i] == '?' )
            {
                if (line[i + 1] == (char) 32 ) //handle ? w/o query
                {
                    query = "";
                }
                else if (line[ i + 1] != (char) 32 ) //handles a real query
                {
                    hasQuery = true;
                    for (int z = i + 1, y = 0; z < i2ndSP; z++, y++)
                    {
                        query[y] = line[z];
                    }
                }
    ...
    //block to check request-target for " and call error if found
    {
        for (int i = i1stSP; i < i2ndSP; i++)
        {
            if (line[i] == (char) 34) 
    ...
    //Block to find and assign Abs_path
    {
        for (int i = i1stSP + 1, j = 0; i < i2ndSP; i++, j++)
        {
            if ( line[i] == '?') //to break out of loop upon hitting ?
            {
                i = i2ndSP;
            }
            else 
        ...
        //Block to check HTTP Version must be HTTP/1.1 also gonna check for the space
    {
        for (int x = strlen(line) - 1, xalpha = strlen(line) - 1, xbeta = xalpha - 11; x > xbeta; x--)
        {
            if ( (x == (xalpha - 0 )) && line[x] != '\n' ) //1
            {
             error(505);
             return false;
            }
            
            if ( (x == (xalpha - 1 )) && line[x] != '\r' ) //2
            {
             error(505);
             return false;
            }
            
            if ( (x == (xalpha - 2 )) && line[x] != (char) 49 ) //3
            {
             error(505);
             return false;
            }
            
            if ( (x == (xalpha - 3 )) && line[x] != '.' ) //4
            {
             error(505);
             return false;
            }
            
            if ( (x == (xalpha - 4 )) && line[x] != (char) 49 ) //5
            {
             error(505);
             return false;
            }
            
            if ( (x == (xalpha - 5 )) && line[x] != '/' ) //6 
            {
             error(505);
             return false;
            }
            
            if ( (x == (xalpha - 6 )) && line[x] != 'P' ) //7
            {
             error(505);
             return false;
            }
            
            if ( (x == (xalpha - 7 )) && line[x] != 'T' ) //8
            {
             error(505);
             return false;
            }
            
            if ( (x == (xalpha - 8 )) && line[x] != 'T' ) //9
            {
             error(505);
             return false;
            }
            if ( (x == (xalpha - 9 )) && line[x] != 'H' ) //10
            {
             error(505);
             return false;
            }
            if ( (x == (xalpha - 10 )) && line[x] != (char) 32 ) //11
            {
             error(505);
             return false;
            }
