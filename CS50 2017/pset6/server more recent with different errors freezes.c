...
bool parse(const char* line, char* abs_path, char* query)
{
 ...
 int RNIndex = strlen(line) - 2;
    ...
    for (int methodCheckIter = 0; methodCheckIter < 3; methodCheckIter++)
    {
    ...
    //Block to check HTTP Version
    {
        bool foundSlash = false;
        char temp = '\0';
        for (int i = i2ndSP + 1, j = strlen(line); i < j; i++ )
        {
            if (line[i] == '/')
            {
                foundSlash = true;
            ...
            else if ( i == j - 1 && foundSlash == false)
            {
                error(505);
                return false;
            }
